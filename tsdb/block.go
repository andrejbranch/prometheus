...
type IndexReader interface {
	...
	// LabelValuesIterator returns an iterator for possible label values which may not be sorted.
...
func (r blockIndexReader) SortedLabelValues(ctx context.Context, name string, matchers ...*labels.Matcher) ([]string, error) {
	...
	if len(matchers) == 0 {
		...
		st, err = r.LabelValues(ctx, name, &storage.LabelHints{}, matchers...)
		if err == nil {
			slices.Sort(st)
		}
	}
...
func (r blockIndexReader) LabelValuesIterator(ctx context.Context, name string) index.StringIter {
	return nil
}
